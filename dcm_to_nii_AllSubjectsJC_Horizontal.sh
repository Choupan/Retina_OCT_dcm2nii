	...
	do
		i=`echo $p | rev | cut -d '/' -f1 | rev`  # `i` is the subject name - Adjust `-f12` based on the folder subdirectories

		for l in ${p}/OD_or_OS_Folders
		do
			j=`echo $l | rev | cut -d '/' -f1 | rev` # `j` is the name of the OD and OS folders - Adjust `-f13` based on the folder subdirectories
			...
			do
				...
				exit"
				# In the resulting Vertical scan the Optical Nerve Head is in the right side od fovea for OD (Left side of nifti file), and in the left side of fovea for OS (Right side of nifti file)
